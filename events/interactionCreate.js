...
if(!int.guild) return
    ...
    if (cmd && ['back', 'clear', 'filter', 'loop', 'pause', 'resume', 'skip', 'stop', 'volume', 'nowplaying', 'save', 'search', 'time','lyrics'].includes(int.commandName)) {
        ...
        fetch(url, { method: "GET", headers: { 
            ...
            if (isVoted == 0) {
                ...
                .setTimestamp()
                .setFooter({ text: 'Music Bot - by KAOS Music ❤️', iconURL: int.user.displayAvatarURL({ dynamic: true }) })
                ...
                if (cmd && DJ.commands.includes(int.commandName)) {
                  ...
                  if(!int.member.permissions.has("MANAGE_GUILD")){
                      ...
                      .setTimestamp()
                      .setFooter({ text: 'Music Bot - by by KAOS Music ❤️', iconURL:int.user.displayAvatarURL({ dynamic: true }) });
    ...
    switch (int.customId) {
       ...
       return int.reply({ content: `No music currently playing. ❌`, embeds: [], components: [], ephemeral: true }).catch(e => {})
          ...
          .setTimestamp()
          .setFooter({ text: 'Music Bot Commands - by KAOS Music ❤️', iconURL: int.user.displayAvatarURL({ dynamic: true }) });
